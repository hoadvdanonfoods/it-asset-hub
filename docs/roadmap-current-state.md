...
**Trạng thái:** đã làm phần cốt lõi

**Đã có:**
- nếu `IS_PRODUCTION=true` và `SECRET_KEY` vẫn là fallback thì fail startup
- nếu `IS_PRODUCTION=true` mà `SESSION_COOKIE_SECURE=false` thì fail startup

**Còn nên làm tiếp:**
- nếu muốn mềm hơn có thể đổi từ fail startup sang health warning, nhưng hiện tại fail startup là an toàn hơn

### 5. Session timeout
**Trạng thái:** đã làm

**Đã có:**
- chuyển sang token có expiry
- timeout theo `SESSION_MAX_AGE_SECONDS`
- cookie có `max_age`
- session cũ bị vô hiệu sau đổi mật khẩu

### 6. Rate limit login
**Trạng thái:** đã làm bản cơ bản

**Đã có:**
- giới hạn số lần login fail theo IP + username
- trả `429` khi vượt ngưỡng
- cấu hình qua env

**Còn nên làm tiếp:**
- cân nhắc state store bền hơn nếu scale nhiều process

### 7. Audit log cho login/logout và action nhạy cảm
**Trạng thái:** đã làm một phần quan trọng

**Đã có:**
- log login success/fail
- log login bị rate limit block
- log logout
- log đổi mật khẩu
- log bulk user/employee actions

**Còn nên làm tiếp:**
- log đổi quyền user rõ hơn nếu cần mức chi tiết sâu hơn
- cân nhắc tách permission riêng cho xem password resource nếu muốn siết hơn nữa
...
- chọn bằng dropdown trong master-data modal create/edit/bulk-edit
- list nhân viên hiển thị tên phòng ban thay vì chỉ hiện id
- vẫn giữ lưu dữ liệu bằng `department_id`

### 9. Hoàn thiện dropdown master-data ở các form liên quan
**Trạng thái:** đã làm một phần

**Đã có:**
- requester department trong incident form đã đổi sang dropdown
...
- log export resources
- không còn nhúng plaintext password trực tiếp vào HTML list view

**Còn nên làm tiếp:**
- permission riêng cho xem password
- review export có lộ password không theo nhu cầu vận hành thực tế
