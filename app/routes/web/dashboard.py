...
def dashboard(request: Request, db: Session = Depends(get_db), current_user=None):
    ...
    for asset in assets:
        ...
        if asset.status == 'active' and not (asset.assigned_user or '').strip():
    ...
    overdue_maintenance = [item for item in maintenance_items if item.next_maintenance_date and item.next_maintenance_date < today and item.asset and item.asset.status not in ('retired', 'disposed')]

    stale_incidents = []
    for item in open_incident_items:
        if not item.reported_at:
            continue
        age_days = (datetime.utcnow() - item.reported_at).days
